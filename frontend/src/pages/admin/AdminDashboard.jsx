...
import { FaDownload, FaSearch, FaFilter, FaBolt, FaChartBar } from "react-icons/fa";
...
function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 md:p-8 dark:bg-slate-950">
      <div className="mx-auto max-w-7xl">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-blue-500">Admin Control Center</p>
            <h1 className="mt-2 text-3xl font-bold text-slate-900 dark:text-white md:text-4xl">Smart Queue Operations</h1>
          </div>

          <button
            onClick={handleExportCsv}
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-3 font-semibold text-white shadow-lg transition hover:bg-blue-700"
          >
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
          {[
            { label: "Total Queues", value: stats.totalQueues },
            { label: "Active Queues", value: stats.activeQueues },
            { label: "Waiting", value: stats.waitingStudents },
            { label: "Serving", value: stats.servingStudents },
            { label: "Completed", value: stats.completedStudents },
            { label: "Cancelled", value: stats.cancelledStudents },
          ].map((item, index) => (
            <motion.div
              key={item.label}
              ...
              className="rounded-2xl bg-white p-5 shadow-lg dark:bg-slate-900"
            >
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-3xl bg-white p-6 shadow-xl dark:bg-slate-900">
            ...
            <form onSubmit={handleCreateQueue} className="grid gap-4 md:grid-cols-2">
              <input
                type="text"
                name="queueName"
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50 p-3 outline-none ring-0 transition focus:border-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
              />
        ...
        <div className="mt-8 rounded-3xl bg-white p-6 shadow-xl dark:bg-slate-900">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-10 pr-3 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                />
              </div>
              ...
              <select
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 px-3 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
              >
          ...
          {queuesLoading ? (
            <div className="rounded-2xl border border-dashed border-slate-300 p-10 text-center text-slate-500 dark:border-slate-700 dark:text-slate-300">
              Loading queues...
            </div>
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 p-10 text-center dark:border-slate-700">
              ...
              <p className="mt-3 text-slate-500 dark:text-slate-300">Try a different search term or reset the filters.</p>
            </div>
          ) : (
                    ...
                    <tr key={queue._id} className="border-t border-slate-200 dark:border-slate-700">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-2">
                          <button onClick={() => handleQueueAction("call-next", queue._id)} disabled={busyQueueId === queue._id} className="rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60">{busyQueueId === queue._id ? "Working..." : "Call Next"}</button>
                          <button onClick={() => handleQueueAction("complete", queue._id)} disabled={busyQueueId === queue._id} className="rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-60">Complete</button>
                          <button onClick={() => handleQueueAction("pause", queue._id)} disabled={busyQueueId === queue._id || queue.status === "paused" || queue.status === "closed"} className="rounded-lg bg-amber-500 px-3 py-2 text-sm font-semibold text-white hover:bg-amber-600 disabled:opacity-60">Pause</button>
                          <button onClick={() => handleQueueAction("resume", queue._id)} disabled={busyQueueId === queue._id || queue.status === "open" || queue.status === "closed"} className="rounded-lg bg-sky-600 px-3 py-2 text-sm font-semibold text-white hover:bg-sky-700 disabled:opacity-60">Resume</button>
                          <button onClick={() => handleQueueAction("close", queue._id)} disabled={busyQueueId === queue._id || queue.status === "closed"} className="rounded-lg bg-rose-600 px-3 py-2 text-sm font-semibold text-white hover:bg-rose-700 disabled:opacity-60">Close</button>
                        </div>
                      </td>
                    </tr>
                  ))}
